...
public class PhotoUploadMiddleware: FastMiddleware{
    ...
    public init(api: MagicsAPI, relativeURL: String, image: UIImage, fileName: String = "photo") {
        self.api = api
    ...
    public func process(router: FastRouter, complete: @escaping EmptyClosure) {
        ...
        data.append(UIImagePNGRepresentation(image.scaled(toWidth: 200) ?? image) ?? Data())
